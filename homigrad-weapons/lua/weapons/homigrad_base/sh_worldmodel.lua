...
function SWEP:PosAngChanges(ply, desiredPos, desiredAng, bNoAdditional, closeanim)
	...
	local inuse = !ply:IsNPC() && (((!ply.InVehicle || !ply:InVehicle()) && self:KeyDown(IN_USE)) || ((ply.InVehicle && ply:InVehicle()) && not self:KeyDown(IN_USE)))
    ...
    //if not (ent ~= ply and not (inuse)) then
		...
		if ent ~= ply then
			...
			local att_Ang = ent:GetBoneMatrix(ent:LookupBone("ValveBiped.Bip01_Head1")):GetAngles()//att.Ang

			ang[3] = ang[3] + (ply:EyeAngles()[3]) + 90//att_Ang:Forward():Dot(vector_up) * 90
		else
...
local function DrawWorldModel(self, force)
	...
	if IsValid(self.worldModel) and willdraw then
		if self:ShouldUseFakeModel() then
			...
			local Time = math.Round(timing,1)
			local Event = self.AnimsSounds and self.AnimsSounds[self.seq] or false
...
function SWEP:WorldModel_Transform(bNoApply, bNoAdditional, model)
	...
	if IsValid(owner) and (owner:IsNPC() or owner:IsPlayer()) then
		...
		local inuse = !owner:IsNPC() && (((!owner.InVehicle || !owner:InVehicle()) && self:KeyDown(IN_USE)) || ((owner.InVehicle && owner:InVehicle()) && not self:KeyDown(IN_USE))) || owner.suiciding || (owner == ent or owner:GetNetVar("lastFake", 0) > CurTime())
