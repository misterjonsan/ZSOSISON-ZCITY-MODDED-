-- "addons\\homigrad\\lua\\initpost\\menu-n-derma\\derma\\cl_menu_panel.lua"
...
local Selects = {
    ...
    {Title = "Discord", Func = function(luaMenu) luaMenu:Close() gui.OpenURL("https://discord.gg/J2pfEBzdBb")  end},
    {Title = "How to play", Func = function(luaMenu) gui.OpenURL("http://zcity-help.ru/zcity_wiki.htm?") end},
    {Title = "Wikipedia", Func = function(luaMenu) luaMenu:Close() gui.OpenURL("http://zcity-help.ru/zcity_wiki.htm?") end},
...
local red_select = Color(0, 0, 0)
-- local Title = markup.Parse("error")

local Pluv = Material("pluv/pluvkid.jpg")

function PANEL:InitializeMarkup()
	local gm = gmod.GetGamemode().Name .. " I|I " .. string.NiceName(zb ~= nil and zb.GetRoundName or game.GetMap())
    ...
    local text = "<font=ZC_MM_Title><colour=33, 33, 33>Z</colour>-City</font>\n<font=ZCity_Small>" .. gm .. "</font>"
    return markup.Parse(text)
end
...
function PANEL:Init()
    ...
    self:SetColorBG(Color(255, 255, 255, 20))
    self:SetDraggable( false )
    ...
    lDock:DockMargin( ScreenScale(30), ScreenScaleH(15), ScreenScale(10), ScreenScaleH(70) )
    lDock.Paint = function(this, w, h)
        ...
        self.Title:Draw(0, ScreenScale(30), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 255, TEXT_ALIGN_LEFT)
    end
    ...
    zteam:SetText("zsosison, zsosison, zsosison, zsosison")
...
function PANEL:AddSelect( pParent, strTitle, fFunc )
    ...
    btn:Dock(BOTTOM)
    btn:DockMargin(0,ScreenScale(1),0,0)
    btn.Func = fFunc
    btn.RColor = Color(255, 255, 255)
    local luaMenu = self 
