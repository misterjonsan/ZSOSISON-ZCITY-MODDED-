...
function WS.DrawText(text, font, posX, posY, color, textAlign)
    draw.DrawText( text, font, posX + 2, posY + 2, ColorAlpha(color_black,WS.Transparent*200) ,textAlign )
    ...
    for i = 0, #Weapons do
        ...
        for Id = 0, #slotTbl do
            ...
            draw.RoundedBox(
                ...
                sizeX,
                sizeH, 
                ColorAlpha(color_black,WS.Transparent*220) 
            )
            draw.RoundedBox(
                0,
                position,
            ...
            surface.SetDrawColor( 255, 255, 255, WS.Transparent*( SelectedWep == wep and 100 or 0 )  )
            surface.SetMaterial( gradient_u )
            surface.DrawTexturedRect( position, (scrH * 0.025) * (Ammout) + (scrH * 0.05) + lastPos, sizeX, sizeH )
            if SelectedWep == wep then
                surface.SetDrawColor( 255, 255, 255, WS.Transparent*255 )
	            surface.DrawOutlinedRect( position, (scrH * 0.025) * (Ammout) + (scrH * 0.05) + lastPos, sizeX, sizeH, 3 )
            end
