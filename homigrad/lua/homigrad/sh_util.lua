...
hook.Add("StartCommand", "HG(StartCommand)", function(ply, cmd)
	...
	if SERVER and inertia_len > 40 then
...
hook.Add("PlayerFootstep", "CustomFootstep2sad", function(ply, pos, foot, sound, volume, rf)
	...
	if CLIENT and ply == lply then
		ViewPunch(Angle(1 * len / 300 * math.max((350 - ply.move) / 50, 1), 0, 0))
	end
...
if CLIENT then
	...
	hook.Add("AdjustMouseSensitivity", "AdjustRunSensivityHUY", function(defaultSensitivity)
		...
		local isrunning = lply:KeyDown(IN_SPEED) and lply:GetVelocity():Length() >= 10 and not lply:Crouching() and not IsValid(lply:GetNWEntity("FakeRagdoll"))
		...
		eyeAngles[1] = 0
		local forwardMoving = math.max(lply:GetVelocity():GetNormalized():Dot(eyeAngles:Forward()), 0.6)
		...
		if isrunning and lply:GetMoveType() ~= MOVETYPE_NOCLIP then
			return 0.5 * math.max(1 / ((org.immobilization or 0) / 30 + 1),0.4) * wepMul * weaponAdjust + brainadjust
		end
		return (math.max(1 / ((org.immobilization or 0) / 30 + 1),0.4) * wepMul) * weaponAdjust + brainadjust
	end)
end
...
function hg.CanUseLeftHand(ply)
	...
	local Car = (IsValid(ply:GetSimfphys()) and ply:GetSimfphys()) or ply:GetVehicle()
