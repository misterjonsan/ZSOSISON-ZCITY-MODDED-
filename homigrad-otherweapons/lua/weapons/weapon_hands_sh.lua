...
function SWEP:Initialize()
	...
	self:SetChoking(false)
	self:SetChokingTarget(NULL)
end

function SWEP:StopChoking()
	self.firstTimeChokePrint = true
	self.firstTimeVictimChokePrint = true
	if IsValid(self:GetChokingTarget()) then
		self:GetChokingTarget().organism.isChoked = false
		self:SetChoking(false)
		self:SetChokingTarget(NULL)
	end
end
...
function SWEP:SetHandPos(noset)
	...
	-- if self:GetChoking() then
	-- 	local victim = self:GetChokingTarget()
	-- 	if victim:LookupBone("ValveBiped.Bip01_Neck1") then
	-- 		local head, anga = victim:GetBonePosition(victim:LookupBone("ValveBiped.Bip01_Neck1"))
	-- 		head = head + anga:Right() + anga:Forward() - anga:Up()
	-- 		local ang = victim:GetBoneMatrix(victim:LookupBone("ValveBiped.Bip01_Neck1")):GetAngles() 

	-- 		ang = Angle(-90,180,0)
			
	-- 		hg.DragHandsToPos(ply, ply:GetActiveWeapon(), head, true, 2, ang:Forward(), Angle(0,-90,45), Angle(0,0,90))
	-- 	end
	-- end
...
function SWEP:SetupDataTables()
	...
	self:NetworkVar("Bool", 6, "Choking")
	self:NetworkVar("Entity", 0, "ChokingTarget")
end
	...
	if IsValid(phys) then
		...
		if SERVER then
			...
			if ply:KeyDown(IN_ZOOM) then
				...
				local ply2 = RagdollOwner(self.CarryEnt) or self.CarryEnt
				local org = ply2.organism
				if (bone == "ValveBiped.Bip01_Neck1" or bone == "ValveBiped.Bip01_Head1") and self:GetOwner().organism.stamina[1] > 50 and not org.noHead then
					if self.firstTimeChokePrint or self.firstTimeChokePrint == nil then
						self.firstTimeChokePrint = false
						self:SetChoking(true)
						self:SetChokingTarget(self.CarryEnt)
						if org ~= nil then
							ply2.organism.isChoked = true
						end
						self:GetOwner():ChatPrint("You grip their neck with both hands and begin to choke them.")
						local selfcopy = self
						self.CarryEnt:CallOnRemove("removechoking",function()
							if not IsValid(selfcopy) then return end
							selfcopy:StopChoking()
						end)
					end
					if self.firstTimeVictimChokePrint or self.firstTimeVictimChokePrint == nil then
						if IsValid(org.owner) and org.owner ~= nil and org.owner ~= NULL and not org.otrub and org.owner:Alive() then
							org.owner:ChatPrint("You are being choked.")
						end
						self.firstTimeVictimChokePrint = false
					end
				else
					self:StopChoking()
				end
				if self:GetChoking() then
					local owner = self:GetOwner()
					local isAssasin = (owner.SubRole == "traitor_assasin" or owner.SubRole == "traitor_assasin_soe" or owner.PlayerClassName == "sc_infiltrator")
					if owner.organism.stamina[1] < 50 then
						self:StopChoking()
					end
					if (self.ChokeThink or 0) < CurTime() then
						self.ChokeThink = CurTime() + (1 / 120) * (isAssasin and 100 or 90)
						phys:ApplyForceCenter(-vector_up * 6000)
						if org ~= nil then
							ply2.organism.o2[1] = math.max(ply2.organism.o2[1] - (isAssasin and 2 or 1), 0)
						end
						if owner.organism ~= nil then
							owner.organism.stamina.subadd = owner.organism.stamina.subadd + (isAssasin and 3 or 3.5)
						end
