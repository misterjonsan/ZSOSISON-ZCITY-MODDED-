-- "addons\\homigrad-otherweapons\\lua\\weapons\\weapon_tpik_traitor_ied.lua"
...
SWEP.WorldModelReal = "models/weapons/zcity/focus/skibidibomba/c_ied_ins.mdl"
SWEP.WorldModelExchange = false

SWEP.AnimList = {
	    -- self:PlayAnim( anim,time,cycling,callback,reverse,sendtoclient )
	["deploy"] = { "draw", 1, false },
    ["attack"] = { "plant", 0.16, false },
	["attack2"] = { "secondary_throw", 1 },
	["idle"] = {"idle", 5, true},
	["det_draw"] = {"det_draw",1,false},
	["det_idle"] = {"det_idle",1,false},
	["det_detonate"] = {"det_detonate",0.5,false}
}
...
if SERVER then
	util.AddNetworkString("HG_SetLH")
end
if CLIENT then
	net.Receive("HG_SetLH",function(_)
...
function SWEP:SetupDataTables()
    self:NetworkVar("Bool", 1, "ShowDetonator")
    self:NetworkVar("Bool", 2, "ShowExplosive")
	self:NetworkVar("Bool", 3, "Planted")
	if SERVER then
		self:SetPlanted(false)
	end
end

function SWEP:HideDetonatorShowExplosive()
	self:SetNWBool("fafa",false)
end

function SWEP:HideExplosiveShowDetonator()
	self:SetNWBool("fafa",true)
end
...
function SWEP:DrawPostWorldModel()
	if self:GetNWBool("fafa",false) ~= true then
		self:GetWM():ManipulateBoneScale(88,vector_origin,false)
		self:GetWM():ManipulateBoneScale(89,vector_full,false)
		for i = 90,98 do
			self:GetWM():ManipulateBoneScale(i,vector_full,false)
		end
	end
	if self:GetNWBool("fafa",false) == true then
		self:GetWM():ManipulateBoneScale(88,vector_full,false)
		self:GetWM():ManipulateBoneScale(89,vector_origin,false)
		for i = 90,98 do
			self:GetWM():ManipulateBoneScale(i,vector_origin,false)
		end
	end
...
function SWEP:Deploy()
	if not self:GetPlanted() then
		self:HideDetonatorShowExplosive()
	else
		if SERVER then
			net.Start("HG_SetLH")
		...
		self:HideExplosiveShowDetonator()
		self:PlayAnim( unpack(self.AnimList["det_draw"]) )
		timer.Simple(self.AnimList["det_draw"][2],function()
			self:PlayAnim( unpack(self.AnimList["det_idle"]) )
		end)
	end
end
...
function SWEP:SecondaryAttack()
	if SERVER then
		if not self.Planted and not self.CantMan then
			self.CantMan = true
			self:PlayAnim( unpack(self.AnimList["attack2"]) )
			timer.Simple(0.15,function()
				...
				bomb:SetModel("models/weapons/w_ied.mdl")
				...
				if IsValid(bomb:GetPhysicsObject()) then
					...
					bomb:GetPhysicsObject():ApplyForceCenter((self:GetOwner():GetAimVector() * 50)*100)
				end
...
function SWEP:Initialize()
	...
	self.WorldModel = "models/weapons/w_ied.mdl"
	self:HideDetonatorShowExplosive()
end
	...
	function SWEP:AttackHuy()
		if not (self.Planted or self.HaveTheBomb or self.PlantedOnSelf) then
			...
			if IsValid(Tr.Entity) and IsValid(Tr.Entity:GetPhysicsObject()) and Tr.Entity:GetPhysicsObject():GetMass() < 500 then
				self:PlayAnim( unpack(self.AnimList["attack"]) )
				timer.Simple(self.AnimList["attack"][2],function()
					self:HideExplosiveShowDetonator()
					self:PlayAnim( unpack(self.AnimList["det_draw"]) )
					self.setlh = false
					if SERVER then
		...
		if (self.nextattackhuy or 0) <= CurTime() and (self.Planted or self.HaveTheBomb or self.PlantedOnSelf) and not self.KABOOM and not self.nuhuh then
			self.nuhuh = true
			self:PlayAnim( unpack(self.AnimList["det_detonate"]) )
			timer.Simple(0.1,function()
